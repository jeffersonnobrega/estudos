...
function exibir(){
    ...
    for(let i=0; i<3;i++){
        tabela += '<tr>' //tr é o código html para linha
        for(let j=0;j<3;j++){
            ...
            switch(tabuleiro[i][j]){
             ...
             tabela += '<td>' + marcador + '</td>' //td é o código html para coluna
        }
...
function checar(){
    ...
    for(let i=0;i<3;i++){
        let soma = 0
        soma = tabela[i][0] + tabuleiro[i][1] + tabuleiro[i][2]
        if(soma == 3 || soma == -3) {
    ...
    for(let i=0;i<3;i++){
        let soma = 0
        soma = tabela[0][1] + tabuleiro[1][i] + tabuleiro[2][i]
        if(soma == 3 || soma == -3) {
            aviso.innerHTML = 'O jogador ' + numeroJogador() + ' ganhou'
        }
    }
    ...
    //diagonal
        ...
        soma = tabela[0][0] + tabuleiro[1][1] + tabuleiro[2][2]
        if(soma == 3 || soma == -3) {
            aviso.innerHTML = 'O jogador ' + numeroJogador() + ' ganhou'
        }
}
